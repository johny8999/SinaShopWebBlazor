    ...
    public class SeriLogger: FrameWork.Infrastructure.ILogger
    {
        public SeriLogger()
        {
            if (Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") == Environments.Development)
            {
                ...
                Log.Logger = new SeriLogConfig().ConfigFile(Serilog.Events.LogEventLevel.Error).CreateLogger();

            }
            else if (Environment.GetEnvironmentVariable("ASPNETCORE_ENVIRONMENT") == Environments.Production)
            {
                Log.Logger = new SeriLogConfig().ConfigSqlServer(Serilog.Events.LogEventLevel.Error).CreateLogger();
            }
            else
            {
                Log.Logger = new SeriLogConfig().ConfigFile(Serilog.Events.LogEventLevel.Error).CreateLogger();
